...
import time
t= time.time()
for sounding in np.arange(start, start + 1):
    ...
    EC = np.flip(np.exp(np.loadtxt('data and simulations/synthetic_model_selection_quasi2D.txt')).reshape(68, -1), axis=1).reshape(-1,
                                                                                                                     1,
                                                                                                                     order='F')
    ...
    np.savetxt('synthetic_COARSE_dclean_1D_hm_sounding'+str(sounding)+'.txt', dclean)
